...
// content of webpage saved inside of ESP microscheme
const char index_html[] PROGMEM = R"rawliteral(
  ...
  <body>
	  <center>
		  <h1 style='font-size:50px; font-weight:bold; color:green;'><span class="reading" style="color:#3908fc;">Now: <span id="date">%DATE%</span></span>
		  </h1>
	  </center><br>
...
const int   gmtOffset_sec      = 0;             // 0 time line
const int   timeline           = 2;             // Yours living timeline
const int   utcOffsetInSeconds = timeline*3600; // ofset time recalculation
char daysOfTheWeek[7][12] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"}; // variable to convert weak days from numbers to words
WiFiUDP ntpUDP;                                 // FiFi ntp UDP initiation object
NTPClient timeClient(ntpUDP, ntpServer, utcOffsetInSeconds);  //NTP Client time sinchronization object from internet
...
char A[20];                                     // LCD string output variable
void setup() { 
  configTime(gmtOffset_sec, utcOffsetInSeconds, ntpServer); // first time sinchronization with NTP server
  Timeline();                                   // fill timeinfo structure after sinchronization, and create text line of todays date and time to output it by request
  dht.begin();                                  // DHT11 sensor enabling 
  pinMode(MQ_PIN, INPUT);                       // Setap of MQ pin for reading data
  pinMode(ledPin, OUTPUT);                      // config LED pin to output
  digitalWrite(ledPin, HIGH);                   // disavling light on LED pin (because is used build in led - it has inverted control)
  lcd.init();
  lcd.backlight();
  lcd.createChar(0, sub_two);
  lcd.setCursor(0, 0);
  ...
  send_events();              // first events sending to web client
  sensors_rod_spausd_led();   // data output dubling to LCD
...
void loop(){  
  ArduinoOTA.handle();                                // OTA chechecking on loop start
...
void Timeline(){
  getLocalTime(&timeinfo);                            // Function fill timeinfo tm type structure after time sinchronization
...
void send_events(){
    ...
    Timeline();                                       // Convert time structure to text line before send it as event to client web page
    // Events sending to client
    events.send("ping",NULL,millis());
    events.send(Time_line,"date",millis());
